...
class pts { // class for manipulaitng and sisplaying polyloops
 ...
 pt[] G = new pt [maxnv];           // geometry table (vertices)
  ...
  //pts drawCeiling() {beginShape(); for (int v=nv/2; v<nv; v++) v(G[v]); endShape(CLOSE); return this;}
  //pts drawFloor() {beginShape(); for (int v=0; v<nv/2; v++) v(G[v]); endShape(CLOSE); return this;}
...
void extruded(){
  for(int k = 0;k<nv;k++)
  {
   pt ex = P(G[k].x,G[k].y,G[k].z+120.0f);
   G[k+nv] = ex;   
  }
  nv*=2;
};
